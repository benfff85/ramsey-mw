...
## Experiment 4c — cyclotomic prime-sweep extended to order 1021 *(2026-07-18, CLOSED)*

Roadmap §3 executed: `cyclotomic_wide`'s row width bumped 512→**1024-bit** (`WORDS` 8→16), re-anchored (Paley(281) still reproduces 0 / 5,979,680 exactly), then swept **all 39 primes ≡ 1 (mod 4) in (461, 1021]** — single core, niced, ~73 min. The ≡ 3 (mod 4) class is skipped as provably empty (Experiment 6 parity theorem). A mono-8-free coloring at any order p would prove **R(8,8) ≥ p+1** — a direct world-record improvement.

**Result — CLOSED, zero hits: 15,000,842 balanced symmetric colorings tested, 0 mono-8-free** across every prime 509…1021. Log: `results/cyclo_wide_sweep_463_1021.log`. **The classical cyclotomic 2-coloring family is now certified empty for every prime order 269–1021** (~22.1M colorings total across Experiments 2/4/4b/4c, plus the ≡3-mod-4 class closed by theorem). Odds continue to fall with order (mono-8 mass grows steeply); the family is closed "through 1000" and re-running higher has negligible expected value.
...
*2026-07-18 addendum (Experiment 4c):* the cyclotomic perimeter now extends to **order 1021** (22.1M colorings total, zero hits). The classical cyclotomic family is closed "through 1000"; the conclusion is unchanged — reaching R(8,8) ≥ 283 still needs a new mathematical idea or vastly more compute on the non-vertex-transitive 282-space, not another construction family within reach of current tooling.
