...
The exact certification removes any remaining hope that *bigger local windows* help. The single highest-upside direction is **structurally-new mono-8-free base families**. **UPDATE (same day) — the computationally-tractable ones at order 281 are now CLOSED** (see `structurally-new-base-investigation.md`): cyclotomic / generalized-Paley 2-colorings on Z₂₈₁ (indices e = 2–14, ~3,700 colorings) yield **only Paley** (all mono-8-free ones tie at mono-7 = 5,979,680); a global circulant hill-climb floors at ~1.3M mono-8; non-cyclic Cayley graphs are impossible at the prime 281; and vertex-transitive constructions are bad at order 282 (~996k floor). What remains is genuinely open — a non-vertex-transitive mono-8-free 281-graph *not* reachable from Paley (its local neighborhood is locked across ~1M exact windows), algebraic/geometric designs with no computational handle near order 281–282, a new mathematical idea, or vastly more compute on the full non-vertex-transitive 282-space. Compute-efficiency lever if more local search is wanted: a batched / in-process MaxSAT solver (z3 subprocess-launch is now the per-window floor).
