...
- **Basin-floor table so far:** original seed → **25,840**; opt_9 → 25,881; opt_2 → 25,996; opt_4 → 26,677 (right-censored). Three of four basins land within 0.6% of the all-time best; the "characteristic floor" distribution is tightening around ~25.9K, and a future basin could still plausibly land *below* 25,840. Remaining samples: opt_7 (live), opt_8.
- **Next rotation pre-staged:** `ROTATION-c15.md` + `rotate_c15.sql` (campaign 15 = opt_8, 27,798); timer ~48 h out (fires ~20:30Z 07-09) with the strengthened 12 h+-wall guard. c15 is the final queued seed — after it walls, compile the program result and decide next steps (opt_4 revisit / 6th seed / M1 reassignment).
